...
const HTTP_URI =
    process.env.NODE_ENV === "production" ? "https://dismal-ugly-pelican.gigalixirapp.com" : "http://localhost:4000";
    ...
    if (!ssr) {
        ...
        link = split(
            ({ query }) => {
                ...
                return (
                    definition.kind === "OperationDefinition" && definition.operation === "subscription" //Si es un query o mutation, utiliza el httpLink //Si es una subscription usa el websocket link
                );
            },
            socketLink,
            link
